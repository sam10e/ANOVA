...
# Example of how to find Confidence Interval with 95% confidence
# find t* for 95% CI with df=5-1=4
qt(.975, df=4)

# Calculate 95% CI for mean bp
99 + 2.776*15.97/sqrt(5)
99 - 2.776*15.97/sqrt(5)

# Below is similar to homework

# Data for Regular Oreos (notation regOreos or y1)
regOreos <- c(7.08, 7.16, 6.99, 7.38, 7.76, 7.91, 7.49, 7.85, 7.00, 7.49, 7.58, 7.44, 7.60, 
              7.65, 7.73, 7.55, 7.44, 7.28, 7.29, 7.11)
...
#95% CI for mean amt of milk absorbed
y1bar + qt(.975, df=(n1-1))*s1/sqrt(n1)
y1bar - qt(.975, df=(n1-1))*s1/sqrt(n1)

# We are 95% confident that the true mean of milk absorbed by Regular Oreos is between 7.31g & 7.57g
# OR we are 95% confident that the mean amt of milk absorbed in 10 sec. by the total population of reg Oreos
# is between 7.31g and 7.57g.

# PREFERRED On average, regular Oreos absorbed 7.44 grams of milk in 10 seconds(95% CI 7.31g to 7.57g) 
# while golden Oreos absorbed only 6.67 grams of milk (95% CI. 6.56 to 6.77).

# Data for the Golden Oreos ... Repeat above for Golden Oreos
# goldenOreos <- c(6.44, 6.71 ...)

# Notes 9/20/2018

# Calculate test statistic for two sample t test (Needs second set of data from Golden Oreos)
y1bar
s1
n1
y2bar
...
sp2
teststat <- (y1bar - y2bar - 0)/(sqrt(sp2/n1 + sp2/n2))
teststat
...
t.test(milkabsgrams~treatment, data=oreos, var.equal=TRUE)
