# Oreo Class Analysis
# Created: 11 Sept 2018
# Data found at
# blades.byu.edu/stat230data/oreoWeights.txt

# Read in data
oreo <- read.table(text="run,experiment,treatment,replicate,milkabsgrams,weightbeforeg,weightafterg
...
# oreos <- read.table("class-prac/oreoWeights.txt", header=TRUE, sep=",")
...
# Oreo weights 2 
oreos2 <- read.table(text="run experiment treatment replicate milkabsgrams weightbeforeg weightafterg
1 25 DoubleStuf 5 6.19 14.9 21.09
2 1 Regular 1 7.08 11.56 18.64
3 55 Golden 15 6.44 11.82 18.26
4 37 DoubleStuf 17 6.22 14.96 21.18
...
meanMAG <- mean(oreos$milkabsgrams)
sdMAG <- sd(oreos$milkabsgrams)

aggregate(milkabsgrams~treatment, data = oreo, FUN = mean)

#make a box plot
boxplot(milkabsgrams~treatment, data = oreo, ylab="Milk Absorbed in 10 Seconds (grams)")

# Dotplot
library(lattice)
dotplot(milkabsgrams~treatment, data = oreo, ylab="Milk Absorbed in 10 Seconds (grams)")
