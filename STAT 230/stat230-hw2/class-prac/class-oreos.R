...
oreosSUB <- subset(oreos, treatment %in% c("Regular", "Golden"))
oreosSUB$treatment <- droplevels(oreosSUB$treatment)

RegMean <- mean(oreos$milkabsgrams[oreos$treatment == "Regular"])
Goldmean <- mean(oreos$milkabsgrams[oreos$treatment == "Golden"])

anova(aov(milkabsgrams~treatment, data = oreosSUB, var.equal = TRUE))
t.test(milkabsgrams~treatment, data = oreosSUB, var.equal = TRUE)
