# Memory experiment from class

# Read in the data
memory <- read.table(text = "Major,Student,Test,Wordlist,Distracter,Score,Student2,Student3
...
Eng,12,48,concrete,math,8,3,48", header = TRUE, sep = ",")
...
memory$Student <- as.factor(memory$Student)

# Plots
boxplot(Score~Wordlist+Distracter, data=memory)
library(lattice)
dotplot(Score~Wordlist+Distracter, data=memory, jitter.y=TRUE)

#Anova
memory_anova <- aov(Score~Wordlist+Distracter+Wordlist*Distracter, data = memory)
anova(memory_anova)

#Assumptions
#Independent (index plot)
plot(memory$Test, memory$Score, type="b")

memory$resids <- resid(memory_anova)
# Normally distributed (qqplot)
qqnorm(memory$resids)
qqline(memory$resids)
# Constant variance (sd ratio)
aggregate(Score~Wordlist+Distracter, data = memory, FUN = sd)

# Check for variance among students
boxplot(Score~Student, data = memory)

# ANOVA including blocking variable
anova(aov(Score~Wordlist+Distracter+Wordlist*Distracter+Student, data = memory))
